extends "res://scripts/ui/combat_sandbox_ui_v49.gd"

# Combat Sandbox v50 — ressenti mobile low-tap, isolé de la pile UI canonique.
...
func _apply_logical_initial_formation_v50() -> void:
    var preferred_slots := {"marec": 1, "mathilde": 2, "anouk": 3, "aurelien": 4}
...
func _sandbox_select_target(index: int) -> void:
    ...
    if action.is_empty():
        show_screen("combat_sandbox")
        return
    if str(action.get("target", "enemy")) == "enemy":
        _sandbox_execute_v50()
        return
    show_screen("combat_sandbox")
...
func _sandbox_execute_v50() -> void:
    ...
    _sandbox_turn_banner_text = "%s termine · %s agit" % [before_name, str(after.get("name", "Veilleur"))] if after_id != before_id else ""
    show_screen("combat_sandbox")
...
func _render_sandbox_flow_hint_v50() -> void:
    ...
    if not action.is_empty():
        ...
        elif target_type == "enemy": text = "Touchez un ennemi pour exécuter"
        elif target_type == "ally": text = "Touchez un allié pour exécuter"
        elif target_type == "self": text = "Action personnelle"
...
func _render_sandbox_feedback_v50() -> void:
    if _sandbox_feedback_text.is_empty(): return
    var panel := PanelContainer.new()
...
func _render_sandbox_turn_banner_v50() -> void:
    if _sandbox_turn_banner_text.is_empty(): return
...
func _feedback_from_result_v50(result: Dictionary) -> String:
    ...
    if str(result.get("kind", "")) == "affliction":
        ...
        return "%s appliquée · %d tour(s)" % [str(AFFLICTION_LABELS.get(str(result.get("affliction", "")), "Affliction")), int(result.get("turns", 0))]
    if result.has("hit"):
        ...
        parts.append("lésion sévère" if severity >= 3 else ("lésion importante" if severity >= 2 else "lésion légère"))
        if str(result.get("functional_loss", "functional")) == "impaired": parts.append("fonction diminuée")
        return " · ".join(parts)
